...
namespace CustomBeatmaps.Patches
{
    ...
    public static class OsuEditorPatch
    {
        ...
        private static void HotReloadBeatmapInfo()
        {
            ...
            //CustomBeatmapLoadingOverridePatch.SetOverrideBeatmap(new CustomSongInfo(_editPath, 9).CustomBeatmaps[0]);
            // Note: this is fucked
            //CustomBeatmaps.OSUSongManager.UpdatePackage(_editPath);
            //var index = CustomBeatmaps.OSUSongManager.Packages.FindIndex(check => check.FolderName == _editPath);
            //var beatmaps = (Dictionary<string, BeatmapInfo>)CustomBeatmaps.OSUSongManager.Packages.ElementAt(index).PkgSongs.Select(s => s.Beatmaps);

            _rhythmController.parser.beatmapPath = _songPath;
